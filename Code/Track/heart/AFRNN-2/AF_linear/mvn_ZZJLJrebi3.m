...
rx(jj,1)=aa*(2*cos(t(jj))-cos(2*t(jj)))+ix0-0.05;
...
% rx(jj,1)=aa*cos(t(jj))+0.025325649451339-0.05;
% ry(jj,1)=aa*sin(3*t(jj))+0.297447874853622;
% rz(jj,1)=-0.065951947126940;
% drx(jj,1)=-aa*sin(t(jj));
% dry(jj,1)=aa*3*cos(3*t(jj));
% drz(jj,1)=0;
% ddrx(jj,1)=-aa*cos(t(jj));
% ddry(jj,1)=-aa*9*sin(3*t(jj)); 
% ddrz(jj,1)=0;
